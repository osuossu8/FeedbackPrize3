...
class CFG:
    EXP_ID = 'L001'
    EXP_Description = 'Create pipeline, run 4fold and then all data training.'
    debug = True # False
    apex = True
    train = True
    sync_bn = True
...
class FeedBackModel(nn.Module):
    def __init__(self, model_name):
        ...
        half_len_encoder_layers = int(len(self.model.encoder.layer)/2)

        # Freeze
        if self.cfg.freezing:
            freeze(self.model.embeddings)
            freeze(self.model.encoder.layer[:half_len_encoder_layers])
...
# all data training
# ===================
def train_one_epoch_for_all_data(rank, model, optimizer, scheduler, dataloader, epoch):
    model.train()
    scaler = GradScaler(enabled=CFG.apex)
    losses = AverageMeter()
...
def train_loop_for_all_data(rank, CFG):
    setup_ddp(rank, CFG.n_gpu)

    set_seed(CFG.seed)

    if rank == 0:
        LOGGER.info(f'-------------all data training-------------')
    ...
    train_dataset = FeedBackDataset(train, CFG.tokenizer, CFG.max_len)
    train_sampler = DistributedSampler(train_dataset, num_replicas=CFG.n_gpu, rank=rank, shuffle=True, seed=CFG.seed, drop_last=True,)
    train_dataloader = DataLoader(train_dataset, batch_size=CFG.batch_size, sampler=train_sampler, pin_memory=True, drop_last=True)
    ...
    for epoch in range(CFG.epochs):
        ...
        train_epoch_loss = train_one_epoch_for_all_data(rank, model, optimizer, scheduler, train_dataloader, epoch)
        ...
        if rank == 0:
            ...
            if train_epoch_loss < best_loss:
                ...
                LOGGER.info(f'Epoch {epoch+1} - Save Best Score: {best_score:.4f} Model')
                torch.save({'model': model.state_dict()},
                            OUTPUT_DIR+f"{CFG.model.replace('/', '-')}_all_data_best.pth")
...
def main():
    ...
    mp.spawn(train_loop_for_all_data, args=(CFG), nprocs=CFG.n_gpu, join=True)
