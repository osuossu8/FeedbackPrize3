...
def train_one_epoch(model, optimizer, scheduler, dataloader, valid_loader, device, epoch, best_score, valid_labels):
    ...
    for step, data in enumerate(dataloader):
        ...
        if (step > 0) & (step % CFG.eval_freq == 0) :
            ...
            valid_epoch_loss, pred, embs = valid_one_epoch(model, valid_loader, device, epoch)
...
def valid_one_epoch(model, dataloader, device, epoch):
    ...
    for step, data in enumerate(dataloader):
        ...
        embs.append(embedding_outputs.to('cpu').numpy())
...
def train_loop(fold):
    ...
    for epoch in range(CFG.epochs):
        ...
        train_epoch_loss, valid_epoch_loss, pred, embs, best_score = train_one_epoch(model, optimizer, scheduler, train_loader, valid_loader, device, epoch, best_score, valid_labels)

        score = get_score(pred, valid_labels)

        elapsed = time.time() - start_time

        LOGGER.info(f'interid embedding shape : {embs.shape}')
